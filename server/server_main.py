...
config_finder = ConfigFinder(is_server=True, ask_for_other_location=None, specific_config=args.rally_configuration)
    ...
    def run(self):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            while True:
                ...
                connection_handler = IncomingConnectionHandler(conn, addr, main_server)
                connection_handler.start()


normal = ConnectionListener(NORMAL_PORT)
normal.start()
if not args.start_locally:
    dyndns = ConnectionListener(DYNDNS_PORT)
    dyndns.start()
else:
    dyndns = None
